...
//added by Fuchen Long, used in the image hash coding
template<typename Dtype>
class TripletConstraintRankingLossLayer : public LossLayer<Dtype>
{
public:
	explicit TripletConstraintRankingLossLayer(const LayerParameter& param)
		:LossLayer<Dtype>(param), diff_(){}
	virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top);
	virtual inline int ExactNumBottomBlobs() const { return 3; }
	virtual inline const char* type() const{ return "TripletConstraintRankingLoss"; }
	virtual inline bool AllowForceBackward(const int bottom_index) const
	{
		return bottom_index != 3;
	}
protected:
   ///@copydoc TripletConstraintRankingLossLayer
   virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
   const vector<Blob<Dtype>*>& top);
   virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
   const vector<bool>& propagate_dowm, const vector<Blob<Dtype>*>& bottom);
   ...
   Blob<Dtype> diff_pow_or_si; // ||F-F+||2
   Blob<Dtype> diff_pow_or_di; // ||F-F-||2
   Blob<Dtype> gradientTriplet;
   Blob<Dtype> gradientConstraint;
   Blob<Dtype> gradient;
};
...
//added by Fuchen Long, used in the weekly supervised learning
//This layer is like the CrossEntropy layer, but this is choose the 
//bigest probability to the target, and It is made for the semi-
//supervised Learning. Please check the paper: "Pseudo-Label: The 
//Simple and Effecient Semi-Supervised Learning Method for Deep
//Neural Networks" for more details.
template<typename Dtype>
class PseudoLabelCrossEntropyLossLayer : public LossLayer<Dtype>
{
public:
	explicit PseudoLabelCrossEntropyLossLayer(const LayerParameter& param)
		: LossLayer<Dtype>(param){}
	virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector <Blob<Dtype>*>& top);
	virtual inline int ExactNumBottomBlobs() const { return 2; }
	virtual inline const char *type() const { return "PseudoLabelCrossEntropyLoss"; }
protected:
	/// @ copydoc PseudoLabelCrossEntropyLossLayer
	virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top);
	virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector <Blob<Dtype>*>& bottom);
	void BigProb_select(const int label, int index, const vector<Blob<Dtype>*>& bottom);
	Dtype Lamda_update();
	Blob<Dtype> combine_target;
	Dtype alpha;
	int batch_size;
	int class_num;
	int max_prob_class;
...
//Added by FuchenLong, used in the weekly supervised learning
//the std is computed for the high rank or low rank judgement
//and at the same time use the Pseudo-Label for the parameters
//fine-tune.

template<typename Dtype>
class StdPseudoLabelEntropyLossLayer : public LossLayer<Dtype>
{
public:
	explicit StdPseudoLabelEntropyLossLayer(const LayerParameter& param)
		: LossLayer<Dtype>(param){}
	virtual void LayerSetUp(const vector<Blob<Dtype>*> & bottom,
		const vector<Blob<Dtype>*>& top);
	virtual inline int ExactNumBottomBlobs() const { return 2; }
	virtual inline const char *type() const { return "StdPseudoLabelEntropyLoss"; }
protected:
	// The same with Pseudo-Label in some degree
	virtual void Forward_cpu(const vector<Blob<Dtype>*> & bottom,
		const vector<Blob<Dtype>*>& top);
	virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
	void get_Std(const Dtype* bottom_data, const int batch_size, const int dim);
	Dtype process_softmax(const Dtype* fVector, const int dim, int buff_index);
	Dtype get_submax_exp(const Dtype* fVector, const int dim, int index);
	Dtype alpha;
	Dtype beta;
	Dtype std_margin;
	...
	Blob<Dtype> combine_target;
	Blob<Dtype> exp_sum;
	Blob<Dtype> exp_submax;
	Blob<Dtype> max_index;
	int class_num;
	int batch_size;
	int pseudo_num;
...
// The source code from XiaoTong in CVPR2015
// "Learning from Messive Noisy Labeled Data for Image Classification"
template <typename Dtype>
...
protected:
	...
	shared_ptr<Layer<Dtype> > softmax_layer_y_;
	shared_ptr<Layer<Dtype> > softmax_layer_z_;
...
/* This loss function is used for the variational auto-encoder.'
   The total loss is combinded the KL Distance and the max-likehood 
   function.
*/

template <typename Dtype>
class VarAutoEncoderLossLayer :public LossLayer<Dtype>
{
public:
	explicit VarAutoEncoderLossLayer(const LayerParameter&param)
		:LossLayer<Dtype>(param){}
	virtual void LayerSetUp(const vector<Blob<Dtype>* >&bottom,
		const vector<Blob<Dtype>*>&top);
	virtual inline int ExactNumBottomBlobs() const { return 4; }
	virtual inline const char *type() const { return "VarAutoEncoderLossLayer"; }
protected:
	int batch_size;
	int count;
	Blob<Dtype> unit_vector;
	virtual void Forward_cpu(const vector<Blob<Dtype>*>&bottom,
		const vector<Blob<Dtype>*>&top);
	virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool> &propagate_down, const vector<Blob<Dtype>*>&bottom);
