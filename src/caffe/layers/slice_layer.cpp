...
void SliceLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  //Dtype* imgdata=NULL;//added by fuchen long to show the image
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    for (int n = 0; n < num_slices_; ++n) {
	  ...
	  //caffe_copy(top_slice_axis * slice_size_, bottom_data + bottom_offset, imgdata);//added by fuchen long to show the image
	  //cv::Mat img(32, 32, CV_8UC3);//added by fuchen long
	  //for (int h = 0; h < 32; ++h)// Dtype to image data
	  //{
		 // uchar*ptr1 = img.ptr<uchar>(h); // image
		 // int img_index = 0;
		 // for (int w = 0; w < 32; ++w)
		 // {
			//  for (int c = 0; c < 3; ++c)
			//  {
			//	  int datum_index1 = (c * 32 + h) * 32 + w;
			//	  static_cast<char>(ptr1[img_index++]) = imgdata[datum_index1];
			//  }
		 // }
	  //}
	  //cv::imshow("image",img);//added by fuchen long
