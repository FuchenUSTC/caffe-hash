...
namespace caffe {
	...
	void DataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		if (this->output_labels_) {
			int classnum = this->layer_param_.data_param().class_num();
		    vector<int> label_shape(1, this->layer_param_.data_param().batch_size());
			vector<int> label_shape_vec(1, classnum*this->layer_param_.data_param().batch_size());
			if (classnum > 1)
			{
				top[1]->Reshape(this->layer_param_.data_param().batch_size(),classnum,1,1);
				this->prefetch_label_.Reshape(label_shape_vec);
			}
			else
			{
	...
	void DataLayer<Dtype>::InternalThreadEntry() {
		...
		for (int item_id = 0; item_id < batch_size; ++item_id) {
			...
			if (datum.encoded()) {
  			...
  			if (this->output_labels_) {
				int display = datum.label();
				if (classnum > 1)  //Added by fuchen long for the label vector
				{
					for (int label_pos = 0; label_pos < classnum; label_pos++)
					{
						if (label_pos == display)
						{
				...
				{
					top_label[item_id] = display;
					if (classnum < 0) top_label[item_id] = display - classnum; //Added by fuchen long for pseudo label
				}
			}
