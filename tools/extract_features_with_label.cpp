...
int feature_extraction_pipeline(int argc, char** argv) {
	...
	*/
	...
	vector<string> LABEL(1, "label");
	for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {

		// extract the label
		const boost::shared_ptr<Blob<Dtype>> label_blob = feature_extraction_net->blob_by_name(LABEL[0]);
		...
		for (int i = 0; i < num_features; ++i) {
			...
			for (int n = 0; n < batch_size; ++n) {
				...
				label_blob_data = label_blob->cpu_data() + label_blob->offset(n);
				*feature_dbs[i] << label_blob_data[0] << " "; // writer the label
